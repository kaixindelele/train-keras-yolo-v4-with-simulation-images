...
import os
import sys
sys.path.append('/home/lyl/Documents/yolov4-tf2-master/robosuite_save_data')
import time
...
def run(args, logs_path=None):
    ...
    array2voc = Array2voc(saved_path="/home/lyl/yolo_data/VOC_small_csv/VOC2007/")
    ...
    for i in range(args.max_episode*2):
        if scene_num >= args.max_episode:
            break
        # 仿真环境改变
        obs = env.reset()
        ...
        s = np.hstack((obs['eef_pos'],
                       ...
                       obs['object-state']))
        step_num = 0
        for j in range(args.max_step*2):
            ...
            img_name = "image%d_%d.jpg" % (scene_num, step_num)
            ...
            xmax = args.image_height - xmax
            xmax, xmin = xmin, xmax
            save_line = [img_name, xmin, ymin, xmax, ymax, 'cube',
                         ]
            [save_line.append(x) for x in six_points_pos_in_pixel]

            insert_pd = pd.DataFrame(data=save_line)
            csv_path = array2voc.saved_path + "Annotations/" + img_name.replace(".jpg", ".csv")
            try:
                insert_pd.to_csv(csv_path, encoding='gbk')

                img_save_path = array2voc.saved_path + array2voc.image_save_path + img_name
            except Exception as e:
                print("save_e:", e)
                continue
            # end save csv!
...
def main():
    ...
    parser.add_argument('--max_episode', type=str, default=2)
    ...
    parser.add_argument('-min_rotate',type=float,default=-80)
    parser.add_argument('-max_rotate',type=float,default=80)

    parser.add_argument('-min_fovy',type=float,default=-80)
