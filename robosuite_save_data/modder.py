...
def rgb2hsv(r, g, b):
    ...
    V = v * 255.0
    return H, S, V
Lower = np.array([0, 43, 46])  # 要识别颜色的下限　默认红色
Upper = np.array([10, 255, 255])
def get_nored():
    while True:
        rgb = [np.random.randint(255), np.random.randint(255), np.random.randint(255)]
        hsv = rgb2hsv(rgb[0], rgb[1], rgb[2])
        if hsv[0] <= Upper[0] and hsv[0] >= Lower[0] and hsv[1] <= Upper[1] and hsv[1] >= Lower[1] \
...
class TextureModder(BaseModder):
    ...
    def random_color_noise(self,names):
        for name in names:
            bitmap = self.get_texture(name).bitmap
            h, w = bitmap.shape[:2]
            np.random.seed(int(time.time() * 10000 % 10000))
            rgb1,rgb2 = get_nored(),get_nored()
            mask = self.random_state.uniform(size=(h, w)) < fraction
            bitmap[..., :] = np.asarray(rgb1)
            bitmap[mask, :] = np.asarray(rgb2)
        self.upload_texture(name)

    def rand_checkboard(self,names,):
        for name in names:
            bitmap = self.get_texture(name).bitmap
            h, w = bitmap.shape[:2]
            # np.random.seed(int(time.time() * 10000 % 10000))
            rgb1,rgb2 = get_nored(),get_nored()
            bitmap[0:h//2,0:w//2]=rgb1
            bitmap[h//2:,0:w//2]=rgb2
            bitmap[0:h//2,w//2:]=rgb2
            bitmap[h//2:,w//2:]=rgb1
            self.upload_texture(name)

    def rand_colornoise(self,names,fraction=0.9):
